...
public class ElementalAffinity {
    ...
    static ElementalAffinity() {
        ...
        foreach (var item in data) {
            ...
            for (int i = 1; i < item.Length; i++) list.Add(AbilityCalculator.StringToElement(item[i]));
            alliances.Add(AbilityCalculator.StringToElement(item[0]), list);
        }
        data = TextReader.ReadSets("ElementalEnemies");
        foreach (var item in data) enemies.Add(AbilityCalculator.StringToElement(item[0]), AbilityCalculator.StringToElement(item[1]));
    }
    ...
    private Element GetNegativeAffinity(Element baseType) {
        ...
        else return RNG.EnumValue<Element>();
    }
}
