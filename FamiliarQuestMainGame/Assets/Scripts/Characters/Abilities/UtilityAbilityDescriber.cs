...
public static class UtilityAbilityDescriber {
    ...
    private static string SpeedPlusDescription(Ability ability, AbilityAttribute attribute) {
        ...
        return "Increases target player's speed by " + ((int)(((float)attribute.FindParameter("degree").value) * 100f)).ToString() + "% for " + duration.ToString() + " seconds.\n";
    }
    ...
    private static string ParalyzeDescription(Ability ability, AbilityAttribute attribute) {
        ...
        if (utilityAbility.targetType == "none") return "Paralyze all enemies within radius " + ((int)attribute.FindParameter("radius").value).ToString() + " for " + ((int)attribute.FindParameter("duration").value).ToString() + " seconds.\n";
        else if (utilityAbility.targetType == "point" && attribute.FindParameter("radius") != null) return "Paralyze all enemies within radius " + ((int)attribute.FindParameter("radius").value).ToString() + " of targeted point for " + ((int)attribute.FindParameter("duration").value).ToString() + " seconds.\n";
        else return "Paralyze targeted enemy for " + ((int)attribute.FindParameter("duration").value).ToString() + " seconds.\n";
    }

    private static string SpeedMinusDescription(Ability ability, AbilityAttribute attribute) {
        var utilityAbility = ability as UtilityAbility;
        if (utilityAbility.targetType == "none") return "Slows all enemies within radius " + ((int)(attribute.FindParameter("radius").value)).ToString() + " by " + (((int)(attribute.FindParameter("degree").value) * 100f)).ToString() + "% for " + ((int)(attribute.FindParameter("duration").value)).ToString() + " seconds.\n";
        else if (utilityAbility.targetType == "point" && attribute.FindParameter("radius") != null) return "Slows all enemies within radius " + ((int)attribute.FindParameter("radius").value).ToString() + " of targeted point by " + (((int)(attribute.FindParameter("degree").value) * 100f)).ToString() + "% for " + ((int)(attribute.FindParameter("duration").value)).ToString() + " seconds.\n";
        else return "Slows targeted enemy by " + ((int)(attribute.FindParameter("degree").value) * 100f).ToString() + "% for " + ((int)attribute.FindParameter("duration").value).ToString() + " seconds.\n";
    }
