...
Shader "VacuumShaders/Advanced Dissolve/Legacy Shaders/Diffuse Detail" {
Properties {
...
struct v2f_surf {
  UNITY_POSITION(pos);
  float4 pack0 : TEXCOORD0; // _MainTex _Detail
...
v2f_surf vert_surf (appdata_full v) {
  ...
  o.pack0.zw = TRANSFORM_TEX(v.texcoord, _Detail);
...
float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.pack0.xy, 0);
  ...
  surfIN.uv_Detail.x = 1.0;
  surfIN.uv_MainTex = IN.pack0.xy;
  surfIN.uv_Detail = IN.pack0.zw;
