...
void vertParticleUnlit (appdata_particles v, out VertexOutput o)
{
 ...
 o.normalWS = UnityObjectToWorldNormal(v.normal);
  ADVANCED_DISSOLVE_INIT_DATA(clipPosition, v.texcoords, v.texcoords1, o)
}
...
float4 alpha = AdvancedDissolveGetAlpha(IN.texcoord.xy, IN.worldPos, IN.normalWS, IN.dissolveUV);		
...
float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.texcoord.xy, 0);
    ...
    #if defined(_EMISSION)
    half3 emission = readTexture (_EmissionMap, IN).rgb * _EmissionColor;
    #else
    half3 emission = 0;
    #endif
